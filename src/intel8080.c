...
static MicroprocError LoadBinary(Microproc *Intel8080, const char *binary_name)
{
...
static void DisassembleInstruction(uint8_t *instruction)
{
    switch(*instruction)
    {
        ...
        case 0x21: printf("LXI H, 0x%04X\n", (instruction[2] << 8) | instruction[1]); break;
        case 0x22: printf("SHLD 0x%04X\n", (instruction[2] << 8) | instruction[1]); break;
        ...
        case 0x31: printf("LXI SP, 0x%04X\n", (instruction[2] << 8) | instruction[1]); break;
        case 0x32: printf("STA 0x%04X\n", (instruction[2] << 8) | instruction[1]); break;
...
static uint8_t ReadByte(uint8_t *memory, uint16_t address)
{
    return memory[address];
}
__attribute__((unused))
static uint16_t ReadWord(uint8_t *memory, uint16_t address)
{
    return ((memory[address + 1] << 8) | memory[address]);
}

static MicroprocError ExecuteInstruction(Microproc *Intel8080)
{
#if defined(DEBUG)
    printf("Registers state:\n");
    printf("PC = 0x%04X\n", Intel8080->pc);
    printf("SP = 0x%04X\n", Intel8080->sp);
    printf("B = 0x%02X\n", Intel8080->registers[B]);
    printf("C = 0x%02X\n", Intel8080->registers[C]);
    printf("D = 0x%02X\n", Intel8080->registers[D]);
    printf("E = 0x%02X\n", Intel8080->registers[E]);
    printf("H = 0x%02X\n", Intel8080->registers[H]);
    printf("L = 0x%02X\n", Intel8080->registers[L]);
    printf("A = 0x%02X\n", Intel8080->registers[A]);
    printf("Carry = 0x%01X\n", (uint8_t)Intel8080->flags.carry);
    printf("Aux carry = 0x%01X\n", (uint8_t)Intel8080->flags.aux_carry);
    printf("Sign = 0x%01X\n", (uint8_t)Intel8080->flags.sign);
    printf("Zero = 0x%01X\n", (uint8_t)Intel8080->flags.zero);
    printf("Parity = 0x%01X\n", (uint8_t)Intel8080->flags.parity);
    printf("Current instruction: ");
    DisassembleInstruction(&Intel8080->memory[Intel8080->pc]);
#endif
    MicroprocError ret = SUCCESSFUL;
    uint8_t instruction = ReadByte(Intel8080->memory, Intel8080->pc++);
    switch(instruction)
    {

        default: break;
    }
    Intel8080->cycles += InstructionCycles[instruction];
    return ret;
}

MicroprocError MicroprocInit(Microproc *Intel8080, const char *binary_name)
{
    if((NULL == Intel8080) || (NULL == binary_name))
    {
        printf("NULL Intel8080 or binary_name parameter\n");
        return NULL_PARAMETER;
    }
    if(Intel8080->init)
    {
        printf("Microprocessor has already been initialized\n");
        return ALREADY_INIT;
    }
    else
    {
        memset(Intel8080, 0, sizeof(Microproc));
        if(SUCCESSFUL == LoadBinary(Intel8080, binary_name))
        {
            Intel8080->init = 1;
        }
    }
...
MicroprocError MicroprocDeinit(Microproc *Intel8080)
{
    if(NULL == Intel8080)
    {
        printf("NULL Intel8080 parameter\n");
        return NULL_PARAMETER;
    }
    if(!Intel8080->init)
    {
...
MicroprocError MicroprocStep(Microproc *Intel8080)
{
    ...
    return ExecuteInstruction(Intel8080);
}
